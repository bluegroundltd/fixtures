 ...
 */
internal class ParameterGroovyValueGenerator {
    ...
    ): String = "${parameter.packageName}.${parameter.typeName}Builder.a${parameter.typeName}Builder().build()"
    ...
    private fun generateSealedValue(
        ...
        return when {
            sealedEntry.isObject -> "${parameter.typeName}.${sealedEntry.name}.INSTANCE"
            sealedEntry.isFixture -> "${parameter.typeName}${sealedEntry.name}Builder.a${parameter.typeName}${sealedEntry.name}().build()"
    ...
    ): String = if (parameter.typeName.contains("Map")) {
        "[:]"
    } else {
        "[]"
    }
